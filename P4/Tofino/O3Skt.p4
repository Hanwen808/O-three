            ...
            if(ig_intr_md.resubmit_flag == 0){
                ...
                if (meta.key_cmp_flag1 == 0) {
                    ...
                    if (meta.key_cmp_flag2 == 0) {
                        ...
                        tb_get_diff_min_arr_seq.apply();
                        tb_calc_cond_pre1.apply();
                        calc_cond_table.apply();
                        if (meta.cond < 256) {
                            if (hdr.myrecord.bucket_value1 < 16) {
                                hdr.myrecord.resubmit_flag = 1;
                                hdr.myrecord.small_tag = 1;
                            } else {
                                //meta.right_shift_min_seq = hdr.myrecord.min_seq_value >> 16;
                                //tb_diff_fen_zi_mu.apply();
                                tb_diff_arr_min_seq.apply();
                                if (hdr.myrecord.right_shift_fen_zi2 < 0x80000000) {
                                    hdr.myrecord.resubmit_flag = 1;
                                    hdr.myrecord.small_tag = 0;
                                }
                            }
                        }
                    }
