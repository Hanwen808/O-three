...
def create_packet_ip_tcp(eth_h, src_ip, dst_ip, sport, dport):
    tcp_data = tcp_header(src_ip, dst_ip, sport, dport)
    source, dest, seq, ack_seq, offset_res, tcp_flags,  window, tcp_checksum , urg_ptr = struct.unpack("!HHLLBBHHH", tcp_data)
    if source not in flow_max_dict:
        flow_max_dict[source] = seq
        flow_o3_cnt_dict[source] = 0
    else:
        if seq <= flow_max_dict[source]:
            flow_o3_cnt_dict[source] += 1
        else:
            flow_max_dict[source] = seq
    return eth_h + ip_header(src_ip, dst_ip, 64, "tcp",1) + tcp_data
...
def save_real_value():
    with open("o3_real.pickle", 'w') as f:
        pickle.dump(flow_o3_cnt_dict, f)
