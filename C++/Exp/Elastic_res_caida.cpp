...
int main() {
    ...
    for (auto iter = mems.begin(); iter != mems.end(); iter ++) {
        ...
        uint32_t stage2_mem = static_cast<uint32_t>(mem * (1-ratio1) * ratio2 * 1024);
        uint32_t stage3_mem = static_cast<uint32_t>(mem * (1-ratio1) * (1-ratio2) * 1024);
        uint32_t M = stage1_mem / 16;
        uint32_t m = static_cast<uint32_t>((stage2_mem * 8.0) / (96.0 + 64.0 + 1.0));
        uint32_t d = 3;
        uint32_t w = stage3_mem / d / 4;
        ...
        for (int jj = 0; jj < EXP_num; jj++) {
            Sketch *skt = new ElasticSketch(M, m, d, w);
            processPackets(skt, dataset, &res_throughput[jj]);
            testQuery(skt, dataset, &res_response_time[jj]);
            std::vector<uint32_t> thresholds{494, 1481, 2470, 3458, 4446};
            int my_cursor = 0;
        ...
        idxmapThresholds[0] = 494;
        idxmapThresholds[1] = 1481;
        idxmapThresholds[2] = 2470;
        idxmapThresholds[3] = 3458;
        idxmapThresholds[4] = 4446;
        for (int file_idx = 0; file_idx < 5; file_idx ++) {
            ofstream fout;
            string outputFilePath = "../Elastic_CAIDA/elastic_" + std::to_string(*iter) + "_" + std::to_string(idxmapThresholds[file_idx]) + ".txt";
