...
int main() {
    ...
    for (auto iter = mems.begin(); iter != mems.end(); iter ++) {
        double ratio = 0.7;
        uint32_t mem = (*iter) * 1024;
        uint32_t stage2_mem = static_cast<uint32_t>(mem * (1 - ratio));
        uint32_t m2 = stage2_mem / 16;
        uint32_t m1 = static_cast<uint32_t>(mem * ratio) / 16;
        int EXP_num = 10;
        ...
        for (int file_idx = 0; file_idx < 5; file_idx ++) {
            ofstream fout;
            string outputFilePath = "../Lean_CAIDA/caida_lean_" + std::to_string(*iter) + "_" + std::to_string(idxmapThresholds[file_idx]) + ".txt";
