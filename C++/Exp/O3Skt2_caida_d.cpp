...
void saveResults(string outputFilePath, Sketch* skt, unordered_map<KEY, uint32_t, KEYHash, KEYEqual>& realFlowInfo, uint32_t T, double res_throughput, double res_response_time) {
    ofstream fout;
...
int main() {
    ...
    std::vector<uint32_t> d_vt = {2, 3, 4, 5, 6};
    for (auto iter = d_vt.begin(); iter != d_vt.end(); iter ++) {
        uint32_t m1 = stage1_mem / *iter / (12+8+1);
        uint32_t m2 = static_cast<uint32_t>((1.0 * stage2_mem) / 12.5);
        Sketch *skt = new O3Skt2(*iter, m1, 4);
        cout << endl;
        cout << "Start processing..." << endl;
        double res_throughput, res_response_time;
        processPackets(skt, dataset, &res_throughput);
        //save the result in files
        cout << endl;
        cout << "save the result in txt ..." << endl;
        string outputFilePath = "../Params/d_caida/o3skt2_32KB_" + std::to_string(*iter) + "_";
        saveResults(outputFilePath, skt, realFlowInfo, 1481, res_throughput, res_response_time);
    }
