...
int main() {
    ...
    for (auto iter = mems.begin(); iter != mems.end(); iter ++) {
        ...
        uint32_t m1 = stage1_mem / d / (12+6+1);
        ...
        for (int file_idx = 0; file_idx < 5; file_idx ++) {
            ofstream fout;
            string outputFilePath = "../O3Skt2_CAIDA/o3skt2_" + std::to_string(*iter) + "_" + std::to_string(idxmapThresholds[file_idx]) + ".txt";
