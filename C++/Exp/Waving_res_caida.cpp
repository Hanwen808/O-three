...
void saveResults(Sketch* skt, unordered_map<KEY, uint32_t, KEYHash, KEYEqual>& realFlowInfo, uint32_t T, double* F1score, double* Precision, double* Recall, double* ARE) {
    ...
    std::cout << T << " " << real_chaos.size() << std::endl;
...
int main() {
    ...
    for (auto iter = mems.begin(); iter != mems.end(); iter ++) {
        ...
        uint32_t m = static_cast<uint32_t>((stage2_mem * 8.0) / (32 + D * (96.0 + 32.0 + 1.0)));
        int EXP_num = 10;
        ...
        for (int jj = 0; jj < EXP_num; jj++) {
            Sketch *skt = new WavingSketch(M, m);
        ...
        for (int file_idx = 0; file_idx < 5; file_idx ++) {
            ofstream fout;
            string outputFilePath = "../Waving_CAIDA/waving_" + std::to_string(*iter) + "_" + std::to_string(idxmapThresholds[file_idx]) + ".txt";
